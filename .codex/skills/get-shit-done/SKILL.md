...
## Command definitions
Each command loads its definition from individual skills in `/home/sandbox/.codex/skills/gsd-<command>/`.
...
- "Task/subagent" → prefer a matching custom agent from `/home/sandbox/.codex/agents` when available; otherwise adopt that role in-place


## Subagents as Skills
GSD subagents are installed as individual skills in `/home/sandbox/.codex/skills/get-shit-done/agents/`. Each agent has its own `SKILL.md` file with specialized instructions.
...
## Paths
Resources are installed under `/home/sandbox/.codex/skills/get-shit-done`. Individual skills are under `/home/sandbox/.codex/skills/gsd-*/`. Use those paths when command content references other files.
