...
<objective>
Create all phases necessary to close gaps identified by `$get-shit-done audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `$get-shit-done add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `$get-shit-done audit-milestone` first.
```
...
**Also available:**
- `$get-shit-done execute-phase {N}` — if plans already exist
...
`$get-shit-done audit-milestone` — re-audit to verify gaps closed
`$get-shit-done complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `$get-shit-done plan-phase` next
</success_criteria>
