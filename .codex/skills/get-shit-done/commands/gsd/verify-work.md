...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for $get-shit-done execute-phase
</objective>
...
8. If issues found:
   ...
   - Present ready status with `/clear` then `$get-shit-done execute-phase`
</process>
...
- $get-shit-done execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- $get-shit-done complete-milestone — skip audit, archive directly
...
$get-shit-done execute-phase {Z} --gaps-only
...
- $get-shit-done plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- $get-shit-done plan-phase {Z} --gaps — retry fix planning with guidance
- $get-shit-done discuss-phase {Z} — gather more context before replanning
...
- [ ] Ready for `$get-shit-done execute-phase` when complete
</success_criteria>
