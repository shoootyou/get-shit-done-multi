...
**After this command:** Run `$get-shit-done plan-phase [N]` to start execution.
...
1. **Verify project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] || { echo "ERROR: No PROJECT.md. Run $get-shit-done new-project first."; exit 1; }
   ```
...
2. **Check for active milestone (ROADMAP.md exists):**
   ...
   **If ACTIVE_MILESTONE:**
   ...
   - options:
     - "Complete current first" — Run $get-shit-done complete-milestone
     - "Continue anyway" — Start new milestone (will archive current)

   If "Complete current first": Exit with routing to `$get-shit-done complete-milestone`
...
`$get-shit-done discuss-phase [N]` — gather context and clarify approach
...
**Also available:**
- `$get-shit-done plan-phase [N]` — skip discussion, plan directly
...
- [ ] User knows next step is `$get-shit-done plan-phase [N]`
