...
arguments:
  ...
  - name: prompt
    ...
    required: true
examples:
  - command: "$get-shit-done invoke-agent gsd-executor 'Implement authentication'"
    description: Invoke executor agent with custom prompt
  - command: "$get-shit-done invoke-agent gsd-planner 'Plan phase 3'"
...
if (!agent) {
  throw new Error(`Agent not found: ${args[0]}. Run $get-shit-done help to see available agents.`);
}
