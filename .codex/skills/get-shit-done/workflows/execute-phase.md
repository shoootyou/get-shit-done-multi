...
**No dependency analysis needed.** Wave numbers are pre-computed during `$get-shit-done plan-phase`.
...
2. **Spawn all autonomous agents in wave simultaneously:**
   ...
   @.codex/skills/get-shit-done/references/tdd.md
   </execution_context>
...
| `gaps_found` | Present gap summary, offer `$get-shit-done plan-phase {phase} --gaps` |
...
`$get-shit-done plan-phase {X} --gaps`
...
- `$get-shit-done verify-work {X}` — manual testing before planning
```

User runs `$get-shit-done plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `$get-shit-done execute-phase {X} --gaps-only`
...
1. Run `$get-shit-done execute-phase {phase}` again
