...
When gaps are found, structure them for consumption by `{{cmdPrefix}}plan-phase --gaps`.
...
The planner (`{{cmdPrefix}}plan-phase --gaps`) reads this gap analysis and creates appropriate plans.
...
Structured gaps in VERIFICATION.md frontmatter for `{{cmdPrefix}}plan-phase --gaps`.
...
**Structure gaps in YAML frontmatter.** The planner (`{{cmdPrefix}}plan-phase --gaps`) creates plans from your analysis.
