...
If missing: Display "No milestones archived yet. Archive your first milestone with {{cmdPrefix}}archive-milestone"
...
if [ ! -f .planning/MILESTONES.md ]; then
  echo "No milestones archived yet. Archive your first milestone with {{cmdPrefix}}archive-milestone"
  exit 0
fi
```
...
Restore a milestone: {{cmdPrefix}}restore-milestone [name]
```

Expected output (when no milestones):
```
No milestones archived yet. Archive your first milestone with {{cmdPrefix}}archive-milestone
```
...
- `{{cmdPrefix}}archive-milestone` - Archive a completed milestone
- `{{cmdPrefix}}restore-milestone` - Restore an archived milestone (future)
- `{{cmdPrefix}}new-milestone` - Start planning a new milestone
