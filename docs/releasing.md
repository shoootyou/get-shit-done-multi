...
**Validate and publish** (choose one method):
```bash
# Method 1: Environment variable (no -- needed)
PUBLISH=true npm run publish-pre 2.0.0-beta.1

# Method 2: Using -- separator
...
5. If validation succeeds, publish (choose one method):
   ```bash
   # Method 1: Environment variable (recommended, no -- needed)
   PUBLISH=true npm run publish-pre 2.0.0-beta.1
