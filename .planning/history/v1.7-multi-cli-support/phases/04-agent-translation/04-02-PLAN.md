...
must_haves:
  truths:
    ...
    - "Performance tracker wired into agent invoker for automatic measurement"
  artifacts:
    ...
    - path: "bin/lib/orchestration/agent-invoker.js"
      provides: "Agent invoker with performance tracking"
      exports: ["invokeAgent", "getPerformanceTracker"]
  key_links:
    ...
    - from: "bin/lib/orchestration/agent-invoker.js"
      to: "bin/lib/orchestration/performance-tracker.js"
      via: "require('./performance-tracker')"
      pattern: "require.*performance-tracker"
---
...
<task type="auto">
  <name>Task 4: Wire performance tracker into agent invoker</name>
  <files>bin/lib/orchestration/agent-invoker.js</files>
  <action>
Integrate PerformanceTracker into agent-invoker.js to measure agent execution time:

1. **Import PerformanceTracker:**
   ```javascript
   const { PerformanceTracker } = require('./performance-tracker');
   ```

2. **Instantiate tracker:**
   - Create module-level instance: `const perfTracker = new PerformanceTracker();`
   - This ensures single tracker instance shared across all invocations

3. **Update invokeAgent() function:**
   - Before adapter invocation:
     ```javascript
     const startMark = perfTracker.startAgent(agentName, cli);
     ```
   
   - After adapter invocation (in try block):
     ```javascript
     const duration = await perfTracker.endAgent(agentName, cli, startMark);
     ```
   
   - Update return object to include duration:
     ```javascript
     return {
       success: true,
   ...
   - In catch block, still call endAgent to record failed execution time:
     ```javascript
     catch (error) {
       await perfTracker.endAgent(agentName, cli, startMark);
       throw error; // re-throw after recording
     }
     ```

4. **Export getPerformanceTracker() function:**
   ```javascript
   ...
   ```
   - This allows external code to access tracker for getAverageTime() queries

**Why:** Wires performance tracking into agent invocation flow, enabling AGENT-09 (benchmark agent performance across CLIs) and providing data for optimization decisions.

**Reference:** Phase 04-02 Task 1 established PerformanceTracker interface (startAgent, endAgent, getAverageTime methods).
  </action>
  <verify>
```bash
# Verify tracker imported and wired
node -e "const {invokeAgent, getPerformanceTracker} = require('./bin/lib/orchestration/agent-invoker.js'); console.log('getPerformanceTracker:', typeof getPerformanceTracker); const t = getPerformanceTracker(); console.log('Tracker:', t.constructor.name);"

# Test invocation records performance
node -e "const {invokeAgent} = require('./bin/lib/orchestration/agent-invoker.js'); invokeAgent('gsd-executor', 'test').then(r => { console.log('Duration recorded:', typeof r.performance.duration === 'number'); console.log('Performance:', r.performance); });"
```

Should output PerformanceTracker instance and invocation result with performance.duration as number.
  </verify>
  <done>
agent-invoker.js imports PerformanceTracker, calls startAgent/endAgent around adapter invocation, returns performance data in result, and exports getPerformanceTracker() for external queries.
  </done>
</task>
