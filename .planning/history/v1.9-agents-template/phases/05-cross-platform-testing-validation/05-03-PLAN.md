...
must_haves:
  ...
  - "E2E runner orchestrates: generation → installation → invocation → report"
...
- Installation tests (Plan 1) - `scripts/test-agent-installation.js`
...
<task name="create-e2e-runner" type="auto">
  <files>scripts/test-cross-platform.js</files>
  <action>
...
```json
"scripts": {
  "test": "node scripts/test-cross-platform.js",
  "test:generation": "node bin/test-agent-generation.js",
  "test:installation": "node scripts/test-agent-installation.js",
...
```bash
npm test
# Should run scripts/test-cross-platform.js
...
└── lib/test-helpers/
    └── cli-invoker.js             # CLI process spawning helper
```
...
Files created:
- `scripts/test-cross-platform.js` - Unified test orchestrator
