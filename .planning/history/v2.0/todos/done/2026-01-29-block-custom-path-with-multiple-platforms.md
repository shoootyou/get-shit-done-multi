...
---
completed: 2026-01-29T11:12
notes: |
  Successfully implemented validation to block --custom-path with multiple platforms:
  - Created bin/lib/validation/cli-validator.js with validateCustomPathWithPlatforms()
  - Added validation to bin/install.js (after custom-path check, before --check-updates)
  - Error message explains what --custom-path is designed for
  - Provides clear solutions: separate commands OR cd + default paths
  - Tested in /tmp isolated directories:
    * Single platform + custom-path: ✓ Works
    * Multiple platforms + custom-path: ✓ Error with helpful message
    * Multiple platforms without custom-path: ✓ Each gets own directory
  - Prevents data loss bug where second platform overwrites first
