...
- Order validations: cheap-first (disk → templates) then expensive-last (paths)
- Stop on prerequisite failures (templates missing → skip path validation)
...
```
bin/lib/preflight/
├── pre-flight-validator.js    # Orchestrator: validateBeforeInstall()
└── error-formatter.js          # formatPreflightReport() - extend error-logger.js
```
 ...
 * 
 * Order: disk → templates → permissions → paths → symlinks
...
export async function validateBeforeInstall(config) {
  ...
  // 4. Batch path validation (expensive: N paths × 8 layers)
...
**Dependency chain:**
```
Templates (required) → blocks → Paths
                    ↘ blocks → Permissions
...
export function formatPreflightReport(errors) {
  const lines = ['', '🚨 Pre-Flight Validation Failed', ''];
...
Disk:
  ✗ Insufficient space: 5MB available
...
- **Expensive-First Ordering:** Running path validation (N × 8 layers) before checking if templates exist. Order by cost: cheap checks first, expensive last.
...
test_insufficient_disk() {
  ...
  if echo "$output" | grep -q "Insufficient disk space"; then
    echo "✓ Disk check error displayed"
  else
    echo "✗ Expected disk error not found"
    exit 1
  fi
  
  if [ $exit_code -eq 0 ]; then
    echo "✗ Installation should have failed"
    exit 1
  fi
  ...
  echo "✓ Test passed: insufficient_disk"
}
```
...
~~Things that couldn't be fully resolved:~~ **All questions resolved - recommendations accepted:**

1. **Node.js < 19 fallback strategy** ✅ RESOLVED
   ...
   - **Decision:** Document Node 19+ requirement, show warning if unavailable (existing code pattern)

2. **Symlink confirmation prompt UX** ✅ RESOLVED
   ...
   - **Decision:** Follow existing CLI patterns (show resolved path, ask "Continue? [y/N]")

3. **Test automation integration** ✅ RESOLVED
   ...
   - **Decision:** Keep bash tests separate in `tests/integration/preflight/` - easier to maintain, more reliable for filesystem testing

4. **Performance baseline** ✅ RESOLVED
   ...
   - **Decision:** Measure in testing, optimize if > 500ms (likely not needed for ~100 files)
