...
must_haves:
  - goal: "User re-runs installer → sees installed version, gets prompted if update available"
    truths:
      - "Platform selection shows inline version status (v2.0.0 → v2.1.0)"
    ...
    wiring:
      - "install.js → version modules for --check-updates mode"
      - "interactive.js → installation-finder + version-checker for inline display"
      - "orchestrator.js → version-checker for downgrade blocking + major warnings"
    key_links:
      - "interactive.js platform selection → formatPlatformOption() for inline display"
      - "orchestrator.js installation start → compareVersions() for update detection"
      - "--check-updates flag → discoverInstallationsWithStatus() for status display"
---
    ...
    **Add check-updates handler (before main flow):**
    ...
    function formatStatusLine(platform, versionStatus, verbose) {
      ...
      if (versionStatus.status === 'update_available') {
        return `⬆ ${displayName} (${versionStatus.updateType} update available)`;
      }
      
      if (versionStatus.status === 'major_update') {
        return `⚠️  ${displayName} (major update available)`;
      }
  ...
  <action>
    ...
    **Discover installations before platform selection (in runInteractive):**
    ...
    export async function runInteractive() {
      const intro = clack.intro(chalk.bold('🚀 Get Shit Done - Multi-Platform Installer'));
...
<task name="checkpoint-manual-testing" type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    **Visual checks:**
    ...
    - [ ] Update arrows show (v2.0.0 → v2.1.0) if applicable
...
Or install a specific version:
  ...
  npx get-shit-done-multi@${versionStatus.installed}
      ...
      if (versionStatus.status === 'major_update' && !options.yes) {
        console.log('');
        console.log(`⚠️  Major version update detected for ${platform}`);
        console.log('');
        console.log(`   ${versionStatus.installed} → ${versionStatus.current}`);
        console.log('');
...
<task name="create-integration-test" type="auto">
  ...
  <action>
    ...
    describe('update-detection integration', () => {
      ...
      it('should detect outdated local installation', async () => {
        ...
        expect(formatted).toContain('1.9.0 → 2.0.0');
  ...
  <verify>
    ...
    # Verify tests passed
    grep -q "✓" /tmp/integration-test.txt && echo "Integration tests passing"
    ```
  </verify>
...
<task name="update-roadmap" type="auto">
  ...
  <action>
    ...
    - Add plan checkboxes:
      ```
      Plans:
      - [ ] 06-01-PLAN.md — Core Version Detection Modules
      - [ ] 06-02-PLAN.md — Interactive CLI Integration & Update Flow
      ```
  </action>
  <verify>
    ```bash
...
2. Outdated installations show arrow (v2.0.0 → v2.1.0)
...
2. ✅ --verbose flag shows discovery details
