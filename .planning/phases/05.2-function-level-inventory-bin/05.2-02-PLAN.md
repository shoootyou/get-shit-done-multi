...
user_setup:
  external_services: []
  manual_steps:
    - "User will confirm function analyses through progressive disclosure (summary → details on request)"
...
1. **Complexity thresholds** → Smart batching: Simple < 5 batched, Complex >= 10 individual
2. **Helper detection** → Skip helpers (2+ heuristics matched) from separate analysis
3. **Confidence calculation** → Flag low-confidence (<100%) functions for extra user review
4. **Call site analysis** → Record direct dependencies only (documented in output)
5. **Parser strategy** → Use @babel/parser primary, acorn fallback (transparent to user)
...
1. ✅ Thresholds hardcoded: batching based on Simple < 5, Complex >= 10
2. ✅ Helpers detected: 3-heuristic algorithm, 2+ matches skipped
3. ✅ Confidence calculated: deduction formula, flags low-confidence functions
4. ✅ Dependencies limited: direct calls only, constraint documented
5. ✅ Parser fallback: @babel/parser → acorn, graceful error handling
