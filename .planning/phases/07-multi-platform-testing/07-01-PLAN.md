...
autonomous: true
must_haves:
  - "Jest test suite with 7 spec validation tests runs successfully"
  ...
  - "__tests__/ directory contains TEST-01, TEST-02, TEST-03 (with FOUN-06), TEST-04, TEST-05, TEST-10"
  - "jest.config.js configured for CommonJS project"
  - "YAML parser compatibility verified across all platforms"
---
...
**From Context decisions:** Sequential platform testing (Copilot → Claude → Codex), isolated directories, hybrid automated/manual approach, test results in JSON format.
...
describe('Frontmatter Validation (TEST-03)', () => {
  ...
  test('Invalid schemas produce clear error messages', async () => {
    ...
    try {
      ...
      const yaml = require('yaml');
      const serialized = yaml.stringify(invalidSpec);
...
**6. Create yaml-parser-compatibility.test.js (TEST-10):**
...
describe('YAML Parser Compatibility (TEST-10)', () => {
  ...
  test('YAML parser works consistently across platforms', async () => {
    ...
    for (const testCase of testCases) {
      expect(() => {
        const parsed = yaml.parse(testCase.yaml);
        expect(parsed).toBeDefined();
        expect(typeof parsed).toBe('object');
      }).not.toThrow();
    }
...
- [x] __tests__/frontmatter-validation.test.js validates schema error messages (FOUN-06)
...
- [x] __tests__/yaml-parser-compatibility.test.js validates YAML parser consistency (TEST-10)
