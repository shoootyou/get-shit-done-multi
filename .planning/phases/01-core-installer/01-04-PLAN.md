...
must_haves:
  ...
  required_wiring:
    - Tests use test-env.js helper from Plan 3
  ...
  key_links:
    ...
    - Human checkpoint verifies installer works correctly
---

# Phase 1, Plan 4: Integration Test Suites & Verification

## Objective

Create comprehensive integration test suites that verify the complete installation flow works end-to-end. Test that all 29 skills and 13 agents install correctly, template variables render properly, error scenarios are handled gracefully, and all Phase 1 requirements are satisfied. Complete with human verification checkpoint to confirm the installer works correctly before marking Phase 1 complete.

## Context

**Why this matters:** Integration tests are the only way to verify the installer actually works. Unit tests for individual modules (from Plan 1) aren't enough - we need to test the complete orchestration flow. The human verification checkpoint ensures the installer works in real-world usage before declaring Phase 1 complete.
...
- From Plan 3: Test infrastructure (vitest, test-env.js helper)
- From repository: .github/skills/ with 29 skills, .github/agents/ with 13 agents

**Decisions from context:**
- Tests use test-env.js helper which enforces /tmp isolation (from Plan 3)
- Tests verify actual filesystem state, not just exit codes (integration testing)
- Human verification checkpoint after automated tests pass (checkpoint:human-verify)
...
**Dependencies:**
- Plans 1, 2, and 3 must complete (need installer + test infrastructure)
