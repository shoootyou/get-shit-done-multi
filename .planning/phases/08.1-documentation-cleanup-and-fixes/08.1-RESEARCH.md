...
**Installation:**
```bash
npm install fs-extra markdown-link-check prompts chalk
...
└── shared/
    ...
    ├── progress-display.js         # Ora spinners
...
async function migrateWithBackup(oldPath, backupPath) {
  ...
  if (!gitignore.includes(path.basename(backupPath))) {
...
1. **Backup naming convention: Timestamped or static?**
   ...
   - Recommendation: Use static name `.old-gsd-system` (simpler), but add timestamp to nested folder like `.old-gsd-system/backup-2026-01-24/`. This allows single backup in .gitignore but keeps history.

2. **Progress indicator style for migration**
   - What we know: Users want detailed progress with indicators
   - What's unclear: Spinner vs step-by-step vs percentage bar
   - Recommendation: Use step-by-step with checkmarks (most informative):
     ```
     ⏳ Detecting old structure...
     ✓ Found old structure in .github/skills/get-shit-done
     ⏳ Creating backup...
     ✓ Backup created: .old-gsd-system
     ⏳ Removing old files...
     ✓ Migration complete
     ```

3. **Scripts subfolder organization**
   - What we know: Context says scripts/ needs subfolders
   - What's unclear: By type (test/, build/) or by function (generation/, validation/)
   - Recommendation: By function (matches current need):
...
4. **Asset SVG banner improvement scope**
   ...
   - Recommendation: Minimal update in this phase:
