...
# Plan 8.1-03: Link Validation & Script Audit

## Objective

Validate all documentation links after file renaming, audit and remove non-essential scripts with user confirmation, update SVG assets, and create removal documentation.
...
- RESEARCH.md: markdown-link-check, depcheck, interactive audit patterns
...
# COMMAND-COMPARISON.md → command-comparison.md
```
...
# get-shit-done-cc → get-shit-done-multi --copilot
```
...
Remove "by TÂCHES" if present (now multi-maintained):
```bash
grep -r "TÂCHES" assets/

# Replace: "by TÂCHES" → "" (remove)
...
<task name="Commit link fixes and script audit" type="auto">
...
commit_as_user "feat(8.1): link validation and script audit
...
```
  ...
  <done>Link validation, script audit, and asset updates committed</done>
</task>
...
1. All internal markdown links are valid ✓
2. Non-essential scripts removed with user confirmation ✓
3. DEPRECATED.md documents removed files ✓
4. SVG assets show current install command and version ✓
5. Package.json scripts reference only existing files ✓
6. External links validated or documented ✓
