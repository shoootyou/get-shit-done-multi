...
- **Node 18→20 migration**: No breaking changes for this codebase (already using stable APIs), just update engines field
...
**Primary recommendation:** Fix package.json "files" array immediately (add templates/, remove unused entries), delete large audit files, validate with npm pack → extract → test workflow.

## Critical Bugs Analysis

### Bug 1: prepublishOnly Script (Line 37)
...
**Problem:** `scripts/build-templates.js` never existed
- Git history confirms: file was never created
- Likely planned for Phase 1 (template generation) but never implemented
- npm publish will fail with "Cannot find module"

**Resolution:** REMOVE the prepublishOnly line entirely
- templates/ already exists and is ready to publish
- No build step needed (templates are already in final form)
- This script reference is orphaned from Phase 1 planning

**Confidence:** HIGH - git log confirms file never existed, scripts/ only contains audit-functions.js

### Bug 2: files Array (Lines 46-54)

**Current state:**
```json
"files": [
  "bin",           // ✓ CORRECT
  "commands",      // ❌ WRONG - doesn't exist
  "get-shit-done", // ❌ WRONG - dev tooling (.github output)
  "agents",        // ❌ WRONG - doesn't exist
  "hooks",         // ❌ WRONG - dev tooling (git hooks)
  "lib-ghcc",      // ❌ WRONG - doesn't exist
  "docs",          // ❌ WRONG - doesn't exist
  "github"         // ❌ WRONG - doesn't exist
]
```

**Problem:** templates/ is MISSING (CRITICAL)
- templates/ contains the core functionality
- Without it, published package cannot copy files to user platforms
- Package would be completely non-functional

**Resolution:** Replace entire files array
```json
"files": [
  "bin",
  "templates"
...
- templates/ = the actual skills/agents to install
- npm auto-includes: package.json, README, LICENSE, CHANGELOG
- Everything else is development tooling or doesn't exist

**Confidence:** HIGH - verified directory existence, analyzed purpose, npm auto-include behavior documented

### Bug 3: Node Version (Line 57)
...
**Problem:** Node 16 is EOL (end-of-life September 2023)
- No security updates
- LTS ended April 2024
- Current LTS is Node 20
...
- All project APIs are stable in both 18 and 20
- No breaking changes for this codebase
- No code changes needed (only package.json update)

**Confidence:** HIGH - Node.js release schedule verified, API compatibility analyzed, no deprecated APIs found in codebase

### Summary of Critical Bugs

| Bug | Severity | Impact | Fix Complexity |
|-----|----------|--------|----------------|
| prepublishOnly | HIGH | npm publish fails | Trivial - delete line |
| files array | CRITICAL | Published package non-functional | Trivial - replace array |
| Node version | LOW | Security/support concern | Trivial - update version |

**All fixes are one-line changes in package.json.**
...
- ❌ templates/ missing (CRITICAL BUG - templates won't be published)
- ✓ bin/ present (correct)
- ❌ commands/, get-shit-done/, agents/, hooks/, lib-ghcc/, docs/, github/ currently listed but need review
...
**Requirements:**
- Shebang line: `#!/usr/bin/env node` (✓ already present in bin/install.js)
...
test -d templates && echo "✓ templates/ present" || echo "✗ templates/ missing"

# Verify bin/ exists
test -d bin && echo "✓ bin/ present" || echo "✗ bin/ missing"

# Check for unwanted files
test -d .github && echo "✗ .github/ present (should not be)" || echo "✓ .github/ not present"
test -d hooks && echo "✗ hooks/ present (should not be)" || echo "✓ hooks/ not present"
test -d coverage && echo "✗ coverage/ present (should not be)" || echo "✓ coverage/ not present"
test -f audit-functions.json && echo "✗ audit files present (should not be)" || echo "✓ audit files not present"
```
...
node -e "import('commander').then(() => console.log('✓ commander'))"
node -e "import('@clack/prompts').then(() => console.log('✓ @clack/prompts'))"
node -e "import('fs-extra').then(() => console.log('✓ fs-extra'))"
```
...
✓ bin/lib/io/file-operations.js already handles EACCES, EPERM
✓ Error messages include path context
✓ Specific error types thrown (permissionDenied, invalidPath)
✗ One empty catch block in bin/lib/platforms/binary-detector.js (line 21)
...
✓ bin/lib/errors/install-error.js defines EXIT_CODES
✓ Uses exit code 1 for errors
✓ Uses exit code 2 for invalid arguments (INVALID_ARGS)
...
- **Fix:** Test with `npm pack → extract → test`
...
- npm pack → extract → test is enough validation
...
try {
  ...
  console.log('  ✓ templates/ present');
  
  // Forbidden directories
  try {
    execSync(`test ! -d ${packageDir}/.github`, { stdio: 'ignore' });
    console.log('  ✓ .github/ not present');
  } catch {
  ...
  try {
    ...
    console.log('  ✓ audit-functions.json not present');
  } catch {
...
- Shebang verification: head -1 bin/install.js (✓ present)
...
4. Test with npm pack → extract → bin/install.js
