...
must_haves:
  ...
  - "Running `node bin/test-agent-installation.js` simulates installation to temp directories"
...
<task name="create-installation-test" type="auto">
  <files>bin/test-agent-installation.js</files>
  <action>
...
- 1 if any test fails
  ...
  <verify>
```bash
node bin/test-agent-installation.js
...
- `bin/test-agent-installation.js` - Validates installation paths and formatting
