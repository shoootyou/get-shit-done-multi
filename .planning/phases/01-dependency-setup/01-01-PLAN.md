...
must_haves:
  ...
  - version_locked: "Commander.js locked to ^14.0.0 in package.json dependencies section"
...
**Validation results:**
- ✅ Commander.js v14 multi-flag parsing tested successfully
- ✅ Prompts TTY detection and multiselect verified
...
<task name="add-commander-dependency" type="auto">
  ...
  <action>
    ...
    1. Install Commander.js with version constraint:
       ```bash
    ...
    **Why ^14.0.0:**
    - Locks to v14.x.x (allows patch updates 14.0.x, 14.1.x, etc.)
    - Prevents breaking changes from v15+ when released
    - Matches project's caret-range pattern (see prompts@^2.4.2)

    **Expected changes:**
    - package.json: New entry under dependencies: `"commander": "^14.0.0"`
    - package-lock.json: Updated with commander details (may deduplicate with existing transitive)
  </action>
  <verify>
    ```bash
    # Verify Commander.js is in dependencies (not devDependencies or transitive)
    grep -A 5 '"dependencies"' package.json | grep commander

    # Verify version is ^14.0.0 or higher
    npm list commander --depth=0
    ```

    **Success criteria:**
    - package.json contains `"commander": "^14.0.0"` under dependencies
    - `npm list commander --depth=0` shows no warnings
    - Exit code 0 from npm install
  </verify>
  <done>
    Commander.js v14+ is installed as direct dependency with caret version range in package.json
  </done>
</task>
  ...
  <action>
    ...
    1. Create test script:
       ...
       try {
         ...
         if (opts.test === true) {
           console.log('✓ Commander.js instantiation and parsing works');
         } else {
           throw new Error('Flag parsing failed');
         }
       } catch (err) {
         console.error('✗ Commander.js failed:', err.message);
         process.exit(1);
       }
       ...
       try {
         ...
         if (typeof prompts === 'function') {
           console.log('✓ Prompts is callable function');
         } else {
           throw new Error('Prompts is not a function');
         }
       } catch (err) {
         console.error('✗ Prompts failed:', err.message);
         process.exit(1);
       }
       
       console.log('\n✅ All dependency verifications passed');
  ...
  <verify>
    ...
    if [ -f /tmp/verify-cli-deps.js ]; then
      ...
      node -e "
        const { Command } = require('commander');
        const prompts = require('prompts');
        console.log('✓ Both libraries importable');
      "
    fi
    ```

    **Success criteria:**
    - Script outputs "✅ All dependency verifications passed"
...
<task name="document-versions" type="auto">
  <files>None (verification only)</files>
  <action>
    Confirm final installed versions match requirements and document for future reference.
    ...
    2. Generate version summary:
       ...
       ✓ DEPS-01: Commander.js v14+ installed (requirement: v14+, actual: $(node -e "console.log(require('commander/package.json').version)"))
       ✓ DEPS-02: Prompts v2.4+ verified (requirement: v2.4+, actual: $(node -e "console.log(require('prompts/package.json').version)"))
  ...
  <verify>
    ...
    [ "$COMMANDER_MAJOR" -ge 14 ] && echo "✓ Commander.js v$COMMANDER_VERSION >= v14" || exit 1
    ...
    [ "$PROMPTS_MAJOR" -ge 2 ] && [ "$PROMPTS_MINOR" -ge 4 ] && echo "✓ Prompts v$PROMPTS_VERSION >= v2.4" || exit 1
    ```
  ...
  <done>
    Version documentation confirms Phase 1 success criteria met: Commander.js v14+ and Prompts v2.4+ are installed and functional
  </done>
</task>
...
1. **Commander.js installed:**
   - Listed in package.json dependencies with `^14.0.0`
   - `npm list commander --depth=0` shows no warnings
...
**Files modified:**
- `package.json` — Commander.js added to dependencies
- `package-lock.json` — Lockfile updated with Commander.js details
