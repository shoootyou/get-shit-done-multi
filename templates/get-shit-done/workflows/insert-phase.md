...
```bash
INIT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js phase insert "${after_phase}" "${description}")
```
