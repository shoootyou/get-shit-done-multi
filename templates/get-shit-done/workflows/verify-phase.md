...
```bash
INIT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js init phase-op "${PHASE_ARG}")
```
...
```bash
node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js verify key-links "$plan")
