...
```bash
INIT=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js init todos)
```
...
```bash
slug=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js generate-slug "$title" --raw)
```
...
```bash
node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
