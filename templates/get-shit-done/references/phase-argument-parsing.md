...
```bash
PHASE_INFO=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node {{PLATFORM_ROOT}}/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
