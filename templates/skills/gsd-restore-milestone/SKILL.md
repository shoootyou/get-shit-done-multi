...
arguments: [{name: version, type: string, required: true, description: Milestone version to restore}]
metadata:
  platform: copilot
...
If exists:
  ...
  Hint: "Archive it first if you want to replace: {{COMMAND_PREFIX}}archive-milestone {version}"
</step>
...
- Archive again: {{COMMAND_PREFIX}}archive-milestone {version}
```
</step>
</process>
