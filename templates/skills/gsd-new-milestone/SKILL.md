...
1. **Verify project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] || { echo "ERROR: No PROJECT.md. Run {{COMMAND_PREFIX}}new-project first."; exit 1; }
   ```
...
2. **Check for active milestone (ROADMAP.md exists):**
   ...
   **If ACTIVE_MILESTONE:**
   ...
   - options:
     - "Complete current first" — Run {{COMMAND_PREFIX}}complete-milestone
     - "Continue anyway" — Start new milestone (will archive current)

   If "Complete current first": Exit with routing to `{{COMMAND_PREFIX}}complete-milestone`
...
- [ ] User knows next step is `{{COMMAND_PREFIX}}plan-phase [N]`
